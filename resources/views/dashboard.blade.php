...
<body>
    ...
    <script>
        async function loadTopViolentCitiesChart() {
            try {
                const res = await fetch('/api/top-violent-cities');
                if (!res.ok) throw new Error('Falha ao buscar dados: ' + res.status);
                const data = await res.json();

                const labels = data.map(d => d.municipio || '(sem nome)');
                const values = data.map(d => parseFloat(d.crimes_por_100k) || 0);

                const ctx = document.getElementById('topViolentCitiesChart').getContext('2d');
                ...
                new Chart(ctx, {
                    ...
                    data: {
                        labels: labels,
                        datasets: [{
                            label: 'Crimes Violentos por 100k habitantes',
                            data: values,
                ...
                document.getElementById('chartErrorViolent').style.display = 'block';
                document.getElementById('chartErrorViolent').textContent = err.message;
                console.error(err);
            }
        }
